...
void processCandidates(const BinInfoMap & segmentMap, Event& ev, candvec& candidates, int ilay, int seedID, const bool debug)
{
  ...
  if (candidates.size() > 0)
  {
    ...
    if (tmp_candidates.size()>static_cast<size_t>(Config::maxCandsPerSeed)) {
      ...
      std::partial_sort(tmp_candidates.begin(),tmp_candidates.begin()+(Config::maxCandsPerSeed-1),tmp_candidates.end(),sortByScore);
      ...
      auto best = std::min_element(candidates.begin(),candidates.end(),sortByScore);
#ifdef TBB
...
void buildTracksBySeeds(const BinInfoMap & segmentMap, Event& ev)
{
  ...
  for (size_t iseed = 0; iseed < track_candidates.size(); ++iseed) {
    ...
    if (cand.size()>0) {
      ...
      auto best = std::min_element(cand.begin(),cand.end(),sortByScore);
      best->setLabel(evt_track_candidates.size());
