{
  ...
  TFile* f1 = TFile::Open("test_cmssw_1x100_"+pt+"GeV_CE_NVU8_NTH1.root");
  TH1F* h1 = (TH1F*) f1->Get("h_MXNH");
  h1->SetLineColor(kBlack);
  h1->SetLineWidth(2);
  if (pt== "10") h1->SetTitle("1x100 muons with pT=10 GeV ");
  if (pt== "1")  h1->SetTitle("1x100 muons with pT=1 GeV");
  ...
  TFile* f2 = TFile::Open("test_cmssw_1x100_"+pt+"GeV_BH_NVU8_NTH1.root");
  ...
  leg->SetBorderSize(0);
  leg->AddEntry(h1,"CloneEngine","L");
  leg->AddEntry(h2,"BestHit","LP");
  leg->Draw();

  c1.SaveAs("nHitsCMSSW_"+pt+"GeV.png");
