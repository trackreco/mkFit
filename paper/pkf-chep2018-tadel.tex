...
\usepackage{xcolor}
\def\mayberemove{\color{orange}}
\def\remove{\color{magenta}}
\def\add{\color{blue}}
...
{\add
Much shortened version of Appendix A.
}
...
that contain the physical dimensions of a layer {\remove($r_{in}$, $r_{out}$, $z_{min}$
...
be selected each time propagation is required in the code{\remove, e.g., for inter /
...
in the endcaps, where the modules on the same blade of a disk overlap considerably{\remove,
