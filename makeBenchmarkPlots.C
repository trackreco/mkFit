void makeBenchmarkPlots(bool isMic = false, bool isCMSSW = false, bool isEndcap = false)
{
  ...
  g_BH_VU->Draw("ALP");
  g_COMB_VU->Draw("LP");
  TLegend* leg_VU = new TLegend(0.60,0.70,0.85,0.85);
  ...
  lvu.Draw();
  TLegend* leg_VU_speedup = new TLegend(0.20,0.70,0.45,0.85);
  ...
  TGraphErrors* g_COMB_TH = (TGraphErrors*) f->Get("g_COMB_TH");
  ...
  g_BH_TH->Draw("ALP");
  g_COMB_TH->Draw("LP");
  TLegend* leg_TH = new TLegend(0.60,0.70,0.85,0.85);
  ...
  lth.Draw();
  TLegend* leg_TH_speedup = new TLegend(0.20,0.70,0.45,0.85);
