...
double UTrd::SafetyFromInside(const UVector3& p, bool aAccurate) const
{
  ...
  if (Inside(p) == kOutside)
  {
    ...
    UUtils::Exception("G4Trd::DistanceToOut(p)", "Notification", UUtils::Warning, 1,
                      "Point p is outside !?");
  }
#endif
...

  {
    switch (side)
    {
      ...
      case kMZ:
        ...
        break;
      default:
        UUtils::Exception("G4Trd::DistanceToOut(p,v,..)", "Notification", UUtils::Warning, 1, "Undefined side for valid surface normal to solid.");
        break;
    }
  }
...
bool UTrd::Normal(const UVector3& p, UVector3& norm) const
{
  ...
  {
#ifdef UDEBUG
    UUtils::Exception("G4Trd::SurfaceNormal(p)", "Notification", UUtils::Warning, 1, "Point p is not on surface !?");
#endif
