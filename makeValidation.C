...
void makeValidation(TString label = "", const Bool_t cmsswComp = false)
{
  ...
  for (UInt_t i = 0; i < rates.size(); i++)
  {
	...
	TGraphAsymmErrors * g_bh  = ((TEfficiency*)f_bh ->Get(Form("%s/%s_%s_%s_build_pt%3.1f",dirs[i].Data(),rates[i].Data(),sORr[i].Data(),vars[j].Data(),ptcuts[p])))->CreateGraph();
	TGraphAsymmErrors * g_std = ((TEfficiency*)f_std->Get(Form("%s/%s_%s_%s_build_pt%3.1f",dirs[i].Data(),rates[i].Data(),sORr[i].Data(),vars[j].Data(),ptcuts[p])))->CreateGraph();
	TGraphAsymmErrors * g_ce  = ((TEfficiency*)f_ce ->Get(Form("%s/%s_%s_%s_build_pt%3.1f",dirs[i].Data(),rates[i].Data(),sORr[i].Data(),vars[j].Data(),ptcuts[p])))->CreateGraph();
	...
	canv->SaveAs(Form("%s_%s_%s_pt%3.1f.png",label.Data(),rates[i].Data(),vars[j].Data(),ptcuts[p]));
