...
make -j 12 CPPUSERFLAGS+=-DCLEAN_SEEDS CXXUSERFLAGS+=-DCLEAN_SEEDS
#this does make clean inside
runBenchmark

export base=SNB_CMSSW_PU70_clean_native_cleanSeed
echo Run CLEAN_SEEDS build with base = ${base}
make -j 12 CPPUSERFLAGS+="-march=native -mtune=native" CXXUSERFLAGS+="-march=native -mtune=native" CPPUSERFLAGS+=-DCLEAN_SEEDS CXXUSERFLAGS+=-DCLEAN_SEEDS
#this does make clean inside
runBenchmark
    ...
    for sV in "sim " "see --cmssw-seeds"; do echo $sV | while read -r sN sO; do
            for section in ECN2 ECN1 BRL ECP1 ECP2; do
                for bV in "BH bh" "STD std" "CE ce"; do echo $bV | while read -r bN bO; do
                        ...
                        nTH=8
                        echo "${oBase}: benchmark [nTH:${nTH}, nVU:8]"
                        time ./mkFit/mkFit --geom CMS-2017 --read --file-name ${!section} --build-${bO} ${sO} --num-thr ${nTH} >& log_${oBase}_NVU8int_NTH${nTH}_benchmark.txt
                    done
                done
            done
        done
