...
for sV in "sim " "see --cmssw-seeds"; do echo $sV | while read -r sN sO; do
		...
		oBase=${base}_${sN}_${bN}
		for nTH in 1 2 4 8 12 16 24 32; do
		    echo "${oBase}: benchmark native [nTH:${nTH}, nVU:8]"
		    time ./mkFit/mkFit --geom CMS-2017 --read --file-name ${fin} --build-${bO} ${sO} --num-thr ${nTH} >& log_${oBase}_NVU8int_NTH${nTH}_benchmark_native.txt
		done
            done
        done
    done
